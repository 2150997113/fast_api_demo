...
- 🎯 Entry points to execute your program using `python -m <fast_api_demo>` or `$ fast_api_demo` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/2150997113/fast_api_demo/branch/main/graph/badge.svg?token=fast_api_demo_token_here)](https://codecov.io/gh/2150997113/fast_api_demo)
[![CI](https://github.com/2150997113/fast_api_demo/actions/workflows/main.yml/badge.svg)](https://github.com/2150997113/fast_api_demo/actions/workflows/main.yml)

Awesome fast_api_demo created by 2150997113

## Install

from source
```bash
git clone https://github.com/2150997113/fast_api_demo fast_api_demo
cd fast_api_demo
make install
```
...
```bash
pip install fast_api_demo
```

## Executing

```bash
$ fast_api_demo run --port 8080
```

or

```bash
python -m fast_api_demo run --port 8080
```

or

```bash
$ uvicorn fast_api_demo:app
```
...
```bash
❯ fast_api_demo --help
Usage: fast_api_demo [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fast_api_demo create-user --help
Usage: fast_api_demo create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fast_api_demo create-user admin admin --superuser
```
...
Run with `fast_api_demo run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fast_api_demo create-user`.
...
-----------------------------------------------------
fast_api_demo/__init__.py              4      0   100%
fast_api_demo/app.py                  16      1    94%
fast_api_demo/cli.py                  21      0   100%
fast_api_demo/config.py                5      0   100%
fast_api_demo/db.py                   10      0   100%
fast_api_demo/models/__init__.py       0      0   100%
fast_api_demo/models/content.py       47      1    98%
fast_api_demo/routes/__init__.py      11      0   100%
fast_api_demo/routes/content.py       52     25    52%
fast_api_demo/routes/security.py      15      1    93%
fast_api_demo/routes/user.py          52     26    50%
fast_api_demo/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from fast_api_demo.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fast_api_demo/default.toml.

### As environment variables
```bash
export fast_api_demo_KEY=value
export fast_api_demo_KEY="@int 42"
export fast_api_demo_KEY="@jinja {{ this.db.uri }}"
export fast_api_demo_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
fast_api_demo_ENV=production fast_api_demo run
```
